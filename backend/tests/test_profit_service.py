...
from app.models.catalog import CommissionRate, SkuCost
...
from app.models.transactions import OrderLine, Return, Shipment
...
def test_commission_hierarchy_prefers_product_over_category(
    ...
    product = make_product(db_session, store, "SKU-1")
    make_commission(db_session, store, rate=D("0.2000"))
    db_session.add(
        CommissionRate(
            store_id=store.id,
...
def test_settlement_actual_wins_over_everything(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-2")
    db_session.add(
        CommissionRate(
            store_id=store.id,
            scope=CommissionScope.PRODUCT,
...
def test_no_commission_rate_returns_none(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-3")
...
def test_future_tariff_is_not_used_for_past_orders(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-4")
    make_commission(db_session, store, rate=D("0.2000"))
    make_commission(db_session, store, rate=D("0.2300"), valid_from=ORDER_DATE + timedelta(days=30))
...
def test_recompute_writes_line_profit(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-10")
    make_commission(db_session, store)
    order = make_order(db_session, store, [(product, 1, D("120.0000"))])
...
def test_cargo_is_allocated_by_desi(db_session: Session, store: Store) -> None:
    ...
    heavy = make_product(db_session, store, "SKU-HEAVY", desi=D("3.00"))
    light = make_product(db_session, store, "SKU-LIGHT", desi=D("1.00"))
    make_commission(db_session, store)
    order = make_order(
        db_session,
        store,
...
def test_service_fee_is_allocated_by_amount(db_session: Session, store: Store) -> None:
    ...
    big = make_product(db_session, store, "SKU-BIG")
    small = make_product(db_session, store, "SKU-SMALL")
    make_commission(db_session, store)
    order = make_order(db_session, store, [(big, 1, D("300.0000")), (small, 1, D("100.0000"))])
...
def test_actual_cargo_cost_is_preferred(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-11")
    make_commission(db_session, store)
    order = make_order(db_session, store, [(product, 1, D("120.0000"))], cargo=D("24.0000"))
...
def test_return_reduces_revenue(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-12")
    make_commission(db_session, store)
    order = make_order(db_session, store, [(product, 2, D("240.0000"))])
...
def test_recompute_logs_revisions(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-13")
    make_commission(db_session, store)
    order = make_order(db_session, store, [(product, 1, D("120.0000"))])
    profit_service.recompute_orders(db_session, order_ids=[order.id])
...
def test_recompute_is_idempotent(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-14")
    make_commission(db_session, store)
    order = make_order(db_session, store, [(product, 1, D("120.0000"))])
...
def test_recompute_pending_only_touches_missing_lines(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-15")
    make_commission(db_session, store)
    make_order(db_session, store, [(product, 1, D("120.0000"))])
...
def test_line_without_product_is_computed_with_warning(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-16")
    make_commission(db_session, store)
    order = make_order(db_session, store, [(product, 1, D("120.0000"))])
    line = db_session.scalar(select(OrderLine).where(OrderLine.order_id == order.id))
    assert line is not None
...
def test_cancelled_order_produces_zero_profit(db_session: Session, store: Store) -> None:
    ...
    product = make_product(db_session, store, "SKU-17")
    make_commission(db_session, store)
    order = make_order(
